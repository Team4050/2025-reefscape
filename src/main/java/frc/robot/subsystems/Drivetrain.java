...
public class Drivetrain extends SubsystemBase {
  ...
  private MecanumDriveKinematics kinematics =
      new MecanumDriveKinematics(
          new Translation2d(0.2794, 0.2794)
              .minus(Constants.Drivetrain.COMOffsetFromWheelbaseCenter),
          new Translation2d(0.2794, -0.2794)
              .minus(Constants.Drivetrain.COMOffsetFromWheelbaseCenter),
          new Translation2d(-0.2794, 0.2794)
              .minus(Constants.Drivetrain.COMOffsetFromWheelbaseCenter),
  ...
  public void configureMotors() {
    ...
    leftSideConfig.CurrentLimits.SupplyCurrentLimit =
        60; // TODO: If drive performance is significantly impacted, return to 70
    leftSideConfig.CurrentLimits.StatorCurrentLimit =
        160; // TODO: Decrease if motor heat is excessive after drive practice
    ...
    rightSideConfig.CurrentLimits.SupplyCurrentLimit =
        60; // TODO: If drive performance is significantly impacted, return to 70
    rightSideConfig.CurrentLimits.StatorCurrentLimit =
        160; // TODO: Decrease if motor heat is excessive after drive practice
  ...
  public void configureModelBasedControl() {
    ...
    mecanumFieldRelativeKalmanFilter =
        new KalmanFilter<N3, N3, N4>(
            N3.instance, N4.instance, mecanumFieldRelativeSystem, stateDev, sensorDev, 0.02);

    mecanumFieldRelativeLQR =
        new LinearQuadraticRegulator<>(
            mecanumFieldRelativeSystem,
  ...
  public void reconfigure() {
    ...
    holonomicDriveController.setTolerance(
        new Pose2d(
            ...
            new Rotation2d(
                Math.toRadians(SmartDashboard.getNumber("Drivetrain Angle Tolerance", 5)))));
    ...
    pathPlannerHolonomicDriveController =
        new PPHolonomicDriveController(
            new PIDConstants(
                ...
                SmartDashboard.getNumber(dashboardPosIZone, 0.3048)),
            new PIDConstants(
  ...
  private double krakenRadToM(double rads) {
    return (rads / Constants.Drivetrain.moduleGearReduction)
  ...
  public Vector<N4> getY() {
    ...
    // Vector<N4> variance = new Vector<N4>(new Matrix<N4, N1>(N4.instance, N1.instance, new
    // double[] {Math.random() * 0.001, Math.random() * 0.01, Math.random() * 0.001, Math.random() *
    // 0.05}));
    // return new
    // Vector<N4>(mecanumFieldRelativeSystem.getC().times(stateSim).plus(mecanumFieldRelativeSystem.getD().times(u))).plus(variance);
    // return VecBuilder.fill(inputToChassisVelocity.get(0), inputToChassisVelocity.get(1),
    // inputToChassisVelocity.get(2), Constants.Sensors.imu.getRate());
    ...
    return VecBuilder.fill(
        ...
        Constants.Sensors.getIMUYawVelocityRads());
  }
  ...
  public void toRobotModuleRelativeSpeeds(ChassisSpeeds fieldRelativeSpeeds) {
    ...
    var ff =
        new ChassisSpeeds(
            Math.signum(combined.vxMetersPerSecond) * 0.03,
            Math.signum(combined.vyMetersPerSecond) * 0.05,
            Math.signum(combined.omegaRadiansPerSecond) * 0.003);
  ...
  public MecanumDriveWheelSpeeds calculateFeedforward(ChassisSpeeds robotRelativeSpeeds) {
    var ff =
        new ChassisSpeeds(
            Math.signum(robotRelativeSpeeds.vxMetersPerSecond) * 0.03,
            Math.signum(robotRelativeSpeeds.vyMetersPerSecond) * 0.05,
            Math.signum(robotRelativeSpeeds.omegaRadiansPerSecond) * 0.003);
  ...
  public void setFieldRelative(
      ChassisSpeeds fieldRelativeSpeeds, ChassisSpeeds robotRelativeFeedforward) {
    Rotation2d rot = getPoseEstimate().getRotation();
    var combined = ChassisSpeeds.fromFieldRelativeSpeeds(fieldRelativeSpeeds, rot);
    var ff =
        new ChassisSpeeds(
  ...
  public void setVoltageFieldRelative(ChassisSpeeds fieldRelativeSpeeds) {
    var chassisSpeeds =
        ChassisSpeeds.fromFieldRelativeSpeeds(fieldRelativeSpeeds, getPoseEstimate().getRotation());
  ...
  public void driveFieldRelative(Pose2d targetPose, double targetLinearVelocity) {
    ...
    var speeds =
        holonomicDriveController.calculate(
            rotationWrappedPose, targetPose, targetLinearVelocity, targetPose.getRotation());
  ...
  public void periodic() {
    ...
    if (useNetworkTables) {
      ...
      velocity.set(
          new double[] {
            velocityField.vxMetersPerSecond,
            velocityField.vyMetersPerSecond,
            velocityField.omegaRadiansPerSecond
          });
